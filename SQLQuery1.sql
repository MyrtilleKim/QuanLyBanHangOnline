...
insert into SANPHAM values('SP0005', N'Bút lông dầu PM=04','DT0001','01', N'Cây', 5000, 'https://res.cloudinary.com/dzpxhrxsq/image/upload/v1648185653/Shopping_onl/but_long_dau_thien_long_pm-04_fbc72b61c6b24523aac15593f7553306_xo43a7.webp')
...
CREATE PROC	TongTien_PhieuGiao
	@mapg varchar(6), @tongtien int OUTPUT
AS
BEGIN
...
DROP PROC SoLuong_SP
CREATE PROC SoLuong_SP
	@madh varchar(6), @masp varchar(6), @soluong int OUTPUT
AS
BEGIN
...
DECLARE @solg int 
DECLARE @kq tinyint
EXEC @kq = SoLuong_SP 'DH0001', 'SP0010', @solg OUTPUT
IF @kq = 0
...
DROP PROC LichSuDH
CREATE PROC LichSuDH 
	@makh varchar(9)
AS
BEGIN
	SELECT DH.MaDH, DH.NgayDat, PG.MaGiao, PG.NgayGiao FROM DONHANG DH LEFT JOIN PHIEUGIAO PG ON DH.MaDH = PG.MaGiao WHERE DH.MaKH = @makh
END
...
DROP PROC ThemSP
CREATE PROC ThemSP
	@masp varchar(6), @tensp nvarchar(100), @madt varchar(6), @malsp varchar(6), @ma
